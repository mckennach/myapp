  ...
  ({ className, text = 'OR', colorClass = 'default' }, ref) => {
    return (
      <div className={cn(`flex center items-center`, className)} ref={ref}>
        <hr className={cn(`m-0 h-px grow border-none`, `bg-foreground/20`)} />
        <span
          className={cn(`text-12 text-tone-2 px-[1rem]`, `text-foreground`)}
        >
          {text}
        </span>
        <hr
          className={cn(
            ...
            colorClass === 'default' ? 'bg-foreground/20' : `bg-foreground/20`
          )}
        />
      </div>
    )
